-- =========================================================
-- V2: Release Manager 초기화
-- 작성일: 2025-11-21
-- 작성자: Claude Code
-- 설명: Release Manager 전체 시스템 초기화
--       - 릴리즈 버전 관리 테이블
--       - 파일 관리 테이블
--       - 누적 패치 관리 테이블
...
-- =========================================================
-- Step 7: Release Version Hierarchy 클로저 테이블 생성
-- =========================================================
...
-- 클로저 테이블 규칙:
-- 1. 모든 노드는 자기 자신과의 관계를 가짐 (depth=0)
-- 2. 버전 순서대로 ancestor-descendant 관계 설정
-- 3. Transitive closure: A->B, B->C 이면 A->C도 포함
...
-- 직접 자식 버전만 조회하는 뷰
CREATE OR REPLACE VIEW v_direct_children AS
SELECT
    a.release_version_id AS parent_id,
    a.version AS parent_version,
    d.release_version_id AS child_id,
    d.version AS child_version,
    h.depth
FROM release_version_hierarchy h
INNER JOIN release_version a ON h.ancestor_id = a.release_version_id
INNER JOIN release_version d ON h.descendant_id = d.release_version_id
WHERE h.depth = 1;

-- 모든 후손 버전 조회하는 뷰
CREATE OR REPLACE VIEW v_all_descendants AS
SELECT
    a.release_version_id AS ancestor_id,
    a.version AS ancestor_version,
    d.release_version_id AS descendant_id,
    d.version AS descendant_version,
    h.depth
FROM release_version_hierarchy h
INNER JOIN release_version a ON h.ancestor_id = a.release_version_id
INNER JOIN release_version d ON h.descendant_id = d.release_version_id
WHERE h.depth > 0
ORDER BY a.version, h.depth, d.version;

-- =========================================================
-- Rollback 방법
-- =========================================================
-- DROP VIEW IF EXISTS v_all_descendants;
-- DROP VIEW IF EXISTS v_direct_children;
-- DROP TABLE IF EXISTS release_version_hierarchy;
