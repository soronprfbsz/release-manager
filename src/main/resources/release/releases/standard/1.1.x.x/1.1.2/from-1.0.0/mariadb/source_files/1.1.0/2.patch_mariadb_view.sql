 ...
 *
 * 2. 기존 뷰의 잘못 선언된 WITH CHECK OPTION 제거
 * - V_PRIV_MCH_INFO, V_PRIV_MCH_INFO_EN
 * - V_PRIV_T_MCH_INFO, V_PRIV_T_MCH_INFO_EN  
 */ 
...
DROP VIEW IF EXISTS V_PRIV_T_MCH_INFO;

CREATE OR REPLACE
ALGORITHM = UNDEFINED VIEW `V_PRIV_T_MCH_INFO` AS
SELECT
    `V`.`MCH_ID` AS `MCH_ID`,
    `V`.`MCH_IP` AS `MCH_IP`,
    `V`.`MCH_NM` AS `MCH_NM`,
    `V`.`SYS_NAME` AS `SYS_NAME`,
    `V`.`MCH_GB_CD` AS `MCH_GB_CD`,
    `V`.`MCH_GB_NM` AS `MCH_GB_NM`,
    `V`.`MCH_TYPE_CD` AS `MCH_TYPE_CD`,
    `V`.`MCH_TYPE_NM` AS `MCH_TYPE_NM`,
    `V`.`DEVICE_TYPE_CD` AS `DEVICE_TYPE_CD`,
    `V`.`MODEL_ID` AS `MODEL_ID`,
    `V`.`MODEL_NM` AS `MODEL_NM`,
    `V`.`NMS_SVR_ID` AS `NMS_SVR_ID`
FROM
    (
        `NMS_DB`.`V_T_MCH_INFO` `V`
    JOIN (
            SELECT
                `D`.`MCH_ID` AS `MCH_ID`
            FROM
            ...
            WHERE
                (
                    ...
                    FROM
                        `NMS_DB`.`USER_INFO`
                    WHERE
                        `USER_INFO`.`USER_ID` = `FC_NMS_GET_TMP_USER_ID`()
                ) = 1
                AND `D`.`MCH_ID` > 0
                OR (
                    SELECT
        ...
        UNION
            ...
            FROM
                (
                    (
                        `NMS_DB`.`USER_PRIV_GRANT_LIST` `A`
                    JOIN `NMS_DB`.`GROUP_INFO` `B`
                    )
                JOIN `NMS_DB`.`GROUP_OBJ_LIST` `C`
                )
            WHERE
                `A`.`DEST_ID` = `B`.`GROUP_ID`
                AND `B`.`GROUP_ID` = `C`.`GROUP_ID`
                AND `B`.`GROUP_CD` = '1'
                AND `A`.`USER_ID` = `FC_NMS_GET_TMP_USER_ID`()
        ) `T`
    )
WHERE `T`.`MCH_ID` = `V`.`MCH_ID`;

###################################################################################################

DROP VIEW IF EXISTS V_PRIV_T_MCH_INFO_EN;

CREATE OR REPLACE
ALGORITHM = UNDEFINED VIEW `V_PRIV_T_MCH_INFO_EN` AS
SELECT
    `V`.`MCH_ID` AS `MCH_ID`,
    `V`.`MCH_IP` AS `MCH_IP`,
    `V`.`MCH_NM` AS `MCH_NM`,
