...
import com.ts.rm.domain.release.mapper.ReleaseVersionDtoMapper;
...
import com.ts.rm.domain.release.util.PatchNoteManager;
import com.ts.rm.domain.release.util.VersionParser;
import com.ts.rm.domain.release.util.VersionParser.VersionInfo;
...
public class ReleaseVersionService {
    ...
    public ReleaseVersionDto.DetailResponse updateVersion(Long versionId,
        ...
        // 버전 정보 업데이트
        releaseVersionRepository.updateVersionInfo(versionId, request.comment(),
                request.isInstall());

        // 최신 데이터 조회
        ReleaseVersion updatedVersion = findVersionById(versionId);

        log.info("Release version updated successfully with versionId: {}", versionId);
        return mapper.toDetailResponse(updatedVersion);
    }
