package com.sb.domain.account.controller;
...
import com.sb.domain.account.enums.AccountStatus;
import com.sb.domain.account.service.AccountService;
import com.sb.global.config.MessageConfig;
import com.sb.global.exception.BusinessException;
import com.sb.global.exception.ErrorCode;
import com.sb.global.exception.GlobalExceptionHandler;
 ...
 *
 * <p>[@WebMvcTest] - Controller 계층만 테스트 - Service는 MockBean으로 대체 - MockMvc로 HTTP 요청/응답
 * 검증
...
class AccountControllerTest {
    ...
    void createAccount_Success() throws Exception {
        // given
        given(accountService.createAccount(any(AccountDto.CreateRequest.class)))
    ...
    void createAccount_DuplicateEmail() throws Exception {
        // given
        given(accountService.createAccount(any(AccountDto.CreateRequest.class)))
                .willThrow(new BusinessException(ErrorCode.ACCOUNT_EMAIL_CONFLICT));
    ...
    void getAccount_Success() throws Exception {
        // given
        given(accountService.getAccountByAccountId(anyLong())).willReturn(detailResponse);
    ...
    void getAllAccounts_Success() throws Exception {
        // given
        given(accountService.getAllAccounts()).willReturn(List.of(simpleResponse));
    ...
    void getAccountsByStatus_Success() throws Exception {
        // given
        given(accountService.getAccountsByStatus(anyString())).willReturn(List.of(simpleResponse));
    ...
    void searchAccounts_Success() throws Exception {
        // given
        given(accountService.searchAccountsByName(anyString())).willReturn(List.of(simpleResponse));
    ...
    void updateAccount_Success() throws Exception {
        ...
        given(accountService.updateAccount(anyLong(), any(AccountDto.UpdateRequest.class)))
                .willReturn(detailResponse);
    ...
    void deactivateAccount_Success() throws Exception {
        ...
        // when & then
        mockMvc.perform(patch("/api/accounts/1/deactivate"))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("success"));
    }
