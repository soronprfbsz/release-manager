...
class ReleaseVersionRepositoryTest {
    ...
    @Test
    @DisplayName("버전 정보 업데이트 - QueryDSL")
    void updateVersionInfo_Success() {
        // given
        ReleaseVersion version = createStandardVersion("1.1.0", 1, 1, 0);
        ReleaseVersion saved = releaseVersionRepository.save(version);

        // when
        long updatedCount = releaseVersionRepository.updateVersionInfo(
                saved.getReleaseVersionId(), "수정된 코멘트", true);
        ...
        ReleaseVersion updated = releaseVersionRepository.findById(saved.getReleaseVersionId())
                .get();
        assertThat(updated.getComment()).isEqualTo("수정된 코멘트");
        assertThat(updated.getIsInstall()).isTrue();
    }
